$(document).ready(function() {    
  ...
  /**
                ...
                success: function(response) {
                    if ('500' === response.status) {
                        ...
                        if (response.propertyId) {
                            var editUrl = base_url + 'host/propertylisting/' + response.propertyId;
   ...
   function saveTabStatus(propertyId, tabId, status)
   {
       $.ajax({
           url: base_url + 'host/saveTabStatus/' + propertyId + '/' + tabId + '/' + status,
           method: 'post',
           dataType: 'json',
           success: function(response) {
          ...
          if (elementId != thisIsVisitedFlagId) {
              if ("1" == $(this).val()) {
                if ("overview" == thisElementPrefix) {
                    if ("1" != $("#overview_is_submitted").val()) {
                ...
                } else if ("manageRooms" == thisElementPrefix) {
                     if ("1" != $("#manageRooms_is_submitted").val()) {
                        $(".add_all_rooms").trigger("click");
                    }                                   
                } else if ("pricing" == thisElementPrefix) {
                     if ("1" != $("#pricing_is_submitted").val()) {
                        $("#save_pricing").trigger("click");
                    }                                   
                } else if ("amenities" == thisElementPrefix) {
                    if ("1" != $("#amenities_is_submitted").val()) {
                        $("#save_amenities").trigger("click");
                    }                                    
                } else if ("listing" == thisElementPrefix) {
                    if ("1" != $("#listing_is_submitted").val()) {
        ...
        $(".is_submitted").each(function() {
            ...
            if ("1" != $(this).val()) {
                if ("overview" == thisElementPrefix) {
                    ...
                    if ("1" != $("#location_is_submitted").val()) {
                        $("#save_location").trigger("click");
                    }                                    
                } else {
                    //
